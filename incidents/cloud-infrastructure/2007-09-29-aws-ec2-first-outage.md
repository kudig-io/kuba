...
| SA（客情危机） | beta 期数据丢失=云计算第一次信任危机，'把数据交给云'的疑虑被点燃；Amazon 提供免费时长补偿并推动 EBS 研发，但数据丢失无法挽回，企业客户进入观望期 |
| SA（技术危机） | 数据持久性优先于可用性——客户应把实例本地盘视为临时存储，坚持'无状态计算+持久化存储'架构；beta 期服务不等于免责，关键业务不应押注未成熟产品 |
